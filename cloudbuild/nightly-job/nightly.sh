...
case $STEP in
  ...
  # Create the cluster
  create_clusters)
    source cloudbuild/e2e-test-scripts/create_dataproc_cluster.sh "$CLUSTER_NAME_SMALL_TEST" "$REGION_SMALL_TEST" "$ZONE_SMALL_TEST" "$PROPERTIES_SMALL_TEST" "$NUM_WORKERS_SMALL_TEST"
    source cloudbuild/e2e-test-scripts/create_dataproc_cluster.sh "$CLUSTER_NAME_UNBOUNDED_TABLE_TEST" "$REGION_UNBOUNDED_TABLE_TEST" "$ZONE_UNBOUNDED_TABLE_TEST" "$PROPERTIES_UNBOUNDED_TABLE_TEST" "$NUM_WORKERS_UNBOUNDED_TABLE_TEST"
    exit
    ;;
