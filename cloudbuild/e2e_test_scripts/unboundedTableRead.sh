...
python3 cloudbuild/python_scripts/insertDynamicPartitions.py -- --now_timestamp="$now" --arg_project=$ARG_PROJECT --arg_dataset=$ARG_DATASET --arg_table=$ARG_TABLE_UNBOUNDED_TABLE --refresh_interval=$REFRESH_INTERVAL
